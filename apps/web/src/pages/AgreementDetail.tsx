...
export function AgreementDetail() {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <span className="badge badge-muted">Testnet · XLM de prueba</span>
              {myRole && (
      ...
      {!agreement.chainVerified && <StageNotice />}
      <details className="card">
        <summary className="cursor-pointer font-semibold">Revisar versión y contenido exacto del acuerdo</summary>
        <p className="mt-3">Versión {agreement.termsJson?.version ?? 'sin validar'} · SHA-256</p>
        <code className="block break-all text-primary mt-2">{agreement.termsHash}</code>
        <pre className="mt-4 overflow-auto whitespace-pre-wrap break-all text-sm">{agreement.canonicalTerms ?? 'Registro antiguo sin términos canónicos verificables. Crea un nuevo acuerdo.'}</pre>
      </details>
      <div className="grid gap-4 sm:grid-cols-3">
        <div className="card text-center">
          <p className="text-text-muted text-sm">En garantía</p>
          <p className="text-3xl font-bold text-text-primary mt-1">{agreement.chainVerified ? formatXLM(agreement.pending) : 'Sin verificar'}</p>
        </div>
        <div className="card text-center">
          <p className="text-text-muted text-sm">Pagado</p>
          <p className="text-3xl font-bold text-success mt-1">{agreement.chainVerified ? formatXLM(agreement.paid) : 'Sin verificar'}</p>
        </div>
        <div className="card text-center">
          <p className="text-text-muted text-sm">Devuelto</p>
          <p className="text-3xl font-bold text-secondary mt-1">{agreement.chainVerified ? formatXLM(agreement.refunded) : 'Sin verificar'}</p>
        </div>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="card">
            ...
            {agreement.chainVerified && agreement.transactions?.length > 0 ? (
        ...
        <div className="space-y-4">
          ...
          <div className="card">
            ...
            <fieldset disabled={!agreement.chainVerified} className="space-y-2 disabled:opacity-60">
