...
import { agreementsApi, agentApi } from '../services/api';
...
export function MilestoneReview() {
  ...
  const handleApprove = async () => {
    setError('El pago desde la aplicación está pendiente de integración con Soroban. No se ha enviado ninguna transacción.');
  };
  ...
  const handleAISummary = async () => {
    ...
    setAiLoading(true);
    try {
      const response = await agentApi.summarizeEvidence({
          agreementId: id!,
          milestoneIndex,
          evidenceDescription,
      });
  ...
  if (!agreement.chainVerified) return <div className="space-y-4"><StageNotice /><button className="btn-outline" onClick={() => navigate(`/agreement/${id}`)}>Volver al acuerdo</button></div>;
