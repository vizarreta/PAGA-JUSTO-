...
export function Layout() {
  ...
  React.useEffect(() => {
    const check = () => { void useAuthStore.getState().checkNetwork(); };
    check();
          ...
          <div className="flex h-16 items-center justify-between">
            <div className="flex items-center gap-8">
              <Link to="/" className="flex items-center gap-2" aria-label="PagaJusto - Inicio">
                <svg className="w-8 h-8" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <path d="M8 16 L14 22 L24 10" stroke="white" strokeWidth="3" fill="none" strokeLinecap="round" strokeLinejoin="round"/>
                  <defs>
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                aria-label="Comprobar red. Selecciona Testnet en Freighter."
              >
                <span className="flex items-center gap-1">
                  <span className="w-2 h-2 rounded-full bg-primary" />
                  {network === 'testnet' ? 'Testnet · XLM de prueba' : 'Verifica Testnet en Freighter'}
                </span>
              </button>
