...
import { formatXLM, formatDate } from '../types';
...
export function Resolution() {
  ...
  const remainingBalance = (agreement?.deposited || 0) - (agreement?.paid || 0) - (agreement?.refunded || 0);
  ...
  const handleProposeSettlement = async () => {
    if (!agreement) return;
    const clientAmt = parseFloat(proposalForm.clientAmount || '0');
    const freelancerAmt = parseFloat(proposalForm.freelancerAmount || '0');
    ...
    if (Math.abs(clientAmt + freelancerAmt - remainingBalance) > 0.000001) {
                      ...
                      <div className="grid gap-4 sm:grid-cols-3">
                        ...
                        <div className="text-center p-3 bg-bg-dark rounded-lg">
                          ...
                          <p className="font-mono text-lg text-primary">
                            {formatXLM(proposal.clientAmount + proposal.freelancerAmount)}
                          </p>
                        </div>
                      </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex gap-2">
                    <div className="text-sm text-text-muted flex items-center gap-2">
                      Suma: <strong>{formatXLM((parseFloat(proposalForm.clientAmount || '0') + parseFloat(proposalForm.freelancerAmount || '0')).toString())}</strong> /
